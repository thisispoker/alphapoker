#ifndef SIMULATOR_H
#define SIMULATOR_H

#include "Poco/Notification.h"
#include "Poco/NotificationQueue.h"
#include "Poco/ThreadPool.h"
#include "Poco/Thread.h"
#include "Poco/Runnable.h"
#include "Poco/Mutex.h"
#include "Poco/Random.h"
#include "Poco/AutoPtr.h"
#include <iostream>

using Poco::Notification;
using Poco::NotificationQueue;
using Poco::ThreadPool;
using Poco::Thread;
using Poco::Runnable;
using Poco::FastMutex;
using Poco::AutoPtr;

class Simulator
{
    public:
        static Simulator* Instance()
        {
            if (inst == NULL)
                inst = new Simulator();
            return inst;
        }
        void RealTimeMock()
        {
            NotificationQueue queue;

            // create some alphapoker threads
            Alphapoker alphapoker0("alphapoker0", queue);
            // start alphapoker threads
            ThreadPool::defaultPool().start(alphapoker0);
            // distribute some work
            for (int i = 0; i < 50; ++i)
            {
                queue.enqueueNotification(new WorkNotification(i));
            }

            // wait until queue is empty and all threads are 
            // waiting for new work.
            while (!queue.empty()) Thread::sleep(200);
            Thread::sleep(500);

            // stop all alphapoker threads
            queue.wakeUpAll();
            ThreadPool::defaultPool().joinAll();


        }
        void RealTimeRun()
        {

        }

    private:
        Simulator()
        {
        }
        ~Simulator() {}

        static Simulator* inst;

    private:

};

Simulator* Simulator::inst = NULL;

#endif



class WorkNotification: public Notification
                        // The notification sent to alphapoker threads.
{
    public:
        typedef AutoPtr<WorkNotification> Ptr;

        WorkNotification(int data):
            _data(data)
    {
    }

        int data() const
        {
            return _data;
        }

    private:
        int _data;
};


class Alphapoker: public Runnable
                  // A Alphapoker thread that gets work items
                  // from a NotificationQueue.
{
    public:
        Alphapoker(const std::string& name, NotificationQueue& queue):
            _name(name),
            _queue(queue)
    {
    }

        void run()
        {
            Poco::Random rnd;
            for (;;)
            {
                Notification::Ptr pNf(_queue.waitDequeueNotification());
                if (pNf)
                {
                    WorkNotification::Ptr pWorkNf = pNf.cast<WorkNotification>();
                    if (pWorkNf)
                    {
                        {
                            std::cout << _name << " got work notification " << pWorkNf->data() << std::endl;
                        }
                        Thread::sleep(rnd.next(200));
                    }
                }
                else break;
            }
        }

    private:
        std::string        _name;
        NotificationQueue& _queue;
};

